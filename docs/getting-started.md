...
When a project is attached, LoopTroop adds `/.looptroop/` to the repository-local `.git/info/exclude` file so runtime state stays out of normal Git status and future commits. If `.looptroop` was already tracked before the project was attached, ticket startup is blocked to prevent nested/stale worktree data from being checked out into every new ticket worktree. Clean that repository with:
