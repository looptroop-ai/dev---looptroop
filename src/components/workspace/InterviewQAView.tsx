...
import { useTicketAction, useInterviewQuestions, useSubmitAnswers, useSkipInterview } from '@/hooks/useTickets'
...
export function InterviewQAView({ ticket }: InterviewQAViewProps) {
  const { mutate: performAction, isPending: isCanceling } = useTicketAction()
  ...
  const isBusy = isSubmitting || isSkipping || isCanceling
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      {/* Answered questions history — scrollable */}
      <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-3">
        {categories.map(cat => {
          const catQuestions = questionsByCategory[cat] ?? []
          const answeredInCat = catQuestions.filter(q => answeredIds.has(q.id))
              ...
              {answeredInCat.map(q => (
                <div key={q.id} className="rounded border border-border/50 p-2 bg-muted/30 text-xs">
                  ...
                  <div className="flex items-start gap-2 mt-1">
                    ...
                    <span>{answers[q.id] || <span className="italic text-muted-foreground">Skipped</span>}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="shrink-0 px-4 pb-4 flex flex-col" style={{ minHeight: '120px' }}>
        <PhaseLogPanel phase="WAITING_INTERVIEW_ANSWERS" />
      </div>
    </div>
  )
