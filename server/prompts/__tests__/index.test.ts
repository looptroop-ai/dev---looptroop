...
describe.concurrent('structured prompt hardening', () => {
  ...
  it('keeps PROM10a strict about preserving user answers and outputting only a full interview artifact', () => {
    ...
    expect(gapPrompt).toContain('Treat provided choice options as hints, not an exhaustive truth set')
    expect(gapPrompt).toContain('leave it empty or start it with the exact chosen option label before any explanation')
    expect(gapPrompt).toContain('prefer non-empty `free_text` when constraints, caveats, or an alternative interpretation matter')
