...
export const PROM10a: PromptTemplate = {
  ...
  instructions: [
    ...
    'Answer Encoding: For every filled skipped question, set `answer.skipped: false`, provide a concrete `free_text` and/or `selected_option_ids` consistent with the question `answer_type`, set `answered_by: ai_skip`, and set a non-empty ISO-8601 `answered_at` timestamp. When the answer type is choice-based, populate best-fit canonical `selected_option_ids` using the provided option IDs.',
    ...
    'Choice Hint Rule: Treat provided choice options as hints, not an exhaustive truth set. If the best-fit option is only approximate or misses nuance, capture the actual inferred answer in concise `free_text`; prefer non-empty `free_text` when constraints, caveats, or an alternative interpretation matter.',
    'Choice Free Text Rule: If you keep `free_text` for a choice answer, leave it empty or start it with the exact chosen option label before any explanation.',
