...
export const PROM_CODING: PromptTemplate = {
  ...
  instructions: [
    ...
    'Environment Readiness: Before running quality gates, verify the worktree has the dependencies, toolchains, and generated artifacts required by the project commands. If required executables or packages are missing, install or bootstrap them using the repository\'s standard tooling and lockfiles, then continue. Keep the setup minimal and project-local; do not change dependency versions or project configuration unless the bead explicitly requires it.',
